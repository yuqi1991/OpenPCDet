...
from pcdet.utils import box_coder_utils, common_utils
...
class AnchorHeadSingle(AnchorHeadTemplate):
    ...
    def generate_predicted_boxes(self, batch_size, cls_preds, box_preds, dir_cls_preds=None):
        ...
        if dir_cls_preds is not None:
            ...
            max_value,dir_labels = torch.max(dir_cls_preds, dim=2)

            period = (2 * np.pi / self.model_cfg.NUM_DIR_BINS)
            dir_rot = common_utils.limit_period(
                batch_box_preds[..., 6] - dir_offset, dir_limit_offset, period
            )
            new_heading = dir_rot + dir_offset + period * dir_labels.to(batch_box_preds.dtype)
            new_heading = new_heading.view(1,-1,1)
            batch_box_preds = batch_box_preds[..., :6]
            batch_box_preds = torch.cat([batch_box_preds,new_heading],dim=2)

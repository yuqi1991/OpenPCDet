...
class DRDataset(DatasetTemplate):
    ...
    def include_kitti_data(self, mode):
        if self.logger is not None:
            self.logger.info('Loading DR dataset')
        kitti_infos = []
        ...
        if self.logger is not None:
            self.logger.info('Total samples for DR dataset: %d' % (len(kitti_infos)))
    ...
    def get_label(self, idx):
        label_file = self.root_split_path / 'groundtruth' / ('%s.txt' % idx)
...
def create_dr_infos(dataset_cfg, class_names, data_path, save_path, workers=12):
    ...
    train_filename = save_path / ('dr_infos_%s.pkl' % train_split)
    val_filename = save_path / ('dr_infos_%s.pkl' % val_split)
    trainval_filename = save_path / 'dr_infos_trainval.pkl'
    test_filename = save_path / 'dr_infos_test.pkl'
    ...
    print('DR info train file is saved to %s' % train_filename)
    ...
    print('DR info val file is saved to %s' % val_filename)
